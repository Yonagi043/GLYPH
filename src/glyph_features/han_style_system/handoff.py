...
def _producer_records_at_commit(
    root: Path,
    config_file: Path,
    commit: str,
) -> list[dict[str, str]]:
    ...
    source_root = "src/glyph_features/han_style_system"
    ...
    if result.returncode != 0:
        raise ValueError("unable to enumerate producer files at implementation commit")
    source_paths = sorted(
        path
    ...
    specifications = [
        ...
        ("config", config_path),
        ("config", str(TRUST_ROOT_PATH)),
        *(("schema", path) for path in PRODUCER_SCHEMAS),
        ...
        [
            {
                "role": role,
                "path": path,
                "sha256": _git_blob_sha256(root, commit, path),
            }
            for role, path in specifications
        ],
        key=lambda item: item["path"],
...
def _validate_provenance(root: Path, provenance: dict[str, Any], errors: list[str]) -> None:
    ...
    try:
        _require_commit(root, commit)
        expected = _producer_records_at_commit(
            root,
