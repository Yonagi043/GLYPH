...
当前修订 `research-deliverable-1` 已接入精确排列、构念桥接、成对美观选择及研究判断续轮；本轮源码与本文仅在TASK-05未提交，主目录最新路线另存，两工作树不自动同步：
...
- “三字体六排列与两焦点字体”要求每个内容恰有三款不同字体样张、两款焦点字体均存在，自动生成六排列和两焦点双向，按种子交错各重复周期。不完整集合拒绝；读取顺序不等同时呈现空间位置，删除第三字体还改变集合大小。
- “单图四条件测量桥接”可勾选仅美观、仅高端、美观先双题、高端先双题的所需子集。每材料/重复/所选模式生成独立任务；q3高端题测感知高端定位，aesthetic仍为主要结果。未问项显示“未采集”，不计缺失，不与旧q2混分。单题/双题/题序及原灰的两结果分别呈现，不把高端代填为美观。
- q4“成对美观、粗细核对”和q5“仅成对美观”每任务接收一张A/B图板，允许持平或无法判断。结果页显示字样、对比、位置和选择；数字美观未采集，不换算评分。q5不问粗细。图板成员来源随配置复用，旧研究假设只作背景；改变图板内容或显式计划必须另立版本，不能假定新材料自动组成有效对照。当前图板制作使用局部研究脚本，网页没有通用图板编辑器。
- “沿用冻结的显式任务表”仅复用该表的条件与材料；换材料后应选择合适自动生成模式，或由执行端另登记新显式计划。普通表单新提交标为探索修订，父design_contract存入inherited_contract并标为来源背景，不把父冻结协议冒充新轮；当前范围、预期、停止规则与实际配置仍需一致。
...
- 如果新研究冻结了请求模型条件，执行者另按该条件传`model`并逐份核实际宿主显示名；`persona-next`不是自动模型路由器。W03至W05请求`GPT-6 Astra (copilot)`且实际显示`GPT-6 Astra`，不要把旧Auto结果与之无条件合并。显示名仍不是确定底层构建。
- 实际宿主 JSONL 应使用本会话的 `workspaceStorage/.../chatSessions/<session-id>.jsonl`，不是任意历史研究文件或空的调试主日志。执行 agent 从当前宿主上下文核对位置，调用 `persona-import` 提取真实父/子工具事件与原始返回。2026-09-11这轮使用会话 `6c9df393-88b5-498c-93b6-7dfa06df038a`，换会话不能沿用旧路径冒充新记录。
...
- 新任务默认flat_v1：`research/prompts/<task_id>.txt`、`research/inputs/<完整输入SHA>.<suffix>`。旧缺字段配置仍为nested_v1，已生成任务不移动/覆盖。真实看图必须按任务次序且每图一次，回答材料顺序及q3评分键题序须吻合；任务外搜索或补分不纳入。AB06初版长目录被漏抄后发生越界搜索，旧12调用保留暂停，双侧另立R1，不通过放宽规则来接受失败。
...
`research-deliverable-1`当前入口已验证：W04 `study_be4f94378ed066decbb5992d`和W05 `study_ec35f809bc04b5359b9bf27e`有成对表、原答、图像及判断；最新判断`assessment_d4721e658a3a90c0dbba1df8`可继续到保留q5/explicit/8图的配置。实际导出、六固定包HTTP/SHA/CSV、8服务图SHA、桌面和390px窄屏通过。服务最后PID91987，0.4现30运行/472材料/309调用；94新调用分96标量行与46类别行，W02main六份语义失效保留不作美观效应证据。完整研究内容在主目录docs/commercial_typography_findings_zh.md，状态和全部用量只以主AUTORESEARCH第8节/AR-049为准；不自动执行下一轮。

以下 `construct-scope-ablation-2` 为A17历史记录，不覆盖当前入口：

2026-09-11 `construct-scope-ablation-2`：430目录项=原412+18受控样张；24持久运行含6份零模型交互回归。0.4累计215实际问卷，199合格/461纳入、9失败/7偏离，0任务内重试/待回填；49任务暂停，非失败。0.3的16调用/192评分单列。实际服务PID23673在127.0.0.1:8025，后续使用先核对是否仍在线。

新增AB01（56调用、55合格/149评分）、AB02（40合格单图，30美观/30高端值）、AB06-R1（两表示24合格/24评分）均保存判断与固定包；初版AB06另12调用/6合格/6协议偏离保留，不混R1。最新判断 `assessment_b071fcf6c0cebf1b679f9f6d` 在灰度 `study_dd487802033adbb5fd2efcc3`，参考原色 `study_4ee52d5644d230fa12e4606e`。Restau高端题序差跨表示/重复保持，颜色非零差未重复；不是统一颜色优势、总体等效或人类规律。完整证据与下一问题只在主目录AUTORESEARCH第8/12节维护。

三份新固定包：[AB01](http://127.0.0.1:8025/api/research-exports/study_5f8a9bfa878a53d872b9d514-ee543d681af9022b)、[AB02](http://127.0.0.1:8025/api/research-exports/study_5ea8efd7dd739c02c76ccbf7-f3801cb9113cbf4f)、[AB06-R1两表示](http://127.0.0.1:8025/api/research-exports/study_dd487802033adbb5fd2efcc3-80f01a241eb5f7e6)，HTTP/包内SHA/CSV已核验，不含图片；精确SHA见主路线。实际浏览器验证判断续轮、删旧增新材料、选双题、保存/reload、生成/暂停；修正继承契约后再次通过。两工程配置各4暂停任务、零调用。桌面/390像素窄屏结果与输入可读，表格内部横滚；不把HTTP核验当手机宿主最终另存。最终13项研究局部pytest/3项Node通过，未跑全仓。

以下 `research-usability-2` 为历史阶段记录（当时424目录项、16运行、83调用），不作当前总数：
...
后续路线/实现变化可能改变实时结果摘要，不覆盖这些固定包。桌面/手机结果页及桌面判断续轮保存已实际验证；手机宿主最终另存下载仍是历史未验证项，不用HTTP核验冒充该项通过。两轮未新增真人或外部文献，字体与形态/联想未分离，不称因果证明。完整解释和恢复位置以主根AUTORESEARCH第8/12节最新 `construct-scope-ablation-2` 为准。
