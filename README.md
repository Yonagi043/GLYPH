...
研究相关局部测试、桌面和手机结果页、判断续轮、样张复用与保存重开已验证；C02/T01 内部证据包已核对下载内容、CSV 行数和哈希。手机集成浏览器的最终下载保存仍有历史未验证项，不把 HTTP 下载成功写成该项通过。材料配置、逐步操作和宿主执行接口见 [docs/workbench_local_ops_zh.md](docs/workbench_local_ops_zh.md)。
